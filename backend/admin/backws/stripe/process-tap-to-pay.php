<?php
/**
 * Process Tap to Pay Card Payments (NFC)
 * Simulates NFC card tap payment processing
 */
...
try {
    ...
    error_log("Tap to Pay Request: " . json_encode($input));
    ...
    $description = isset($input['description']) ? $input['description'] : 'Tap to Pay Transaction';
    $test_mode = isset($input['test_mode']) ? $input['test_mode'] : true;
    $card_last4 = isset($input['card_last4']) ? $input['card_last4'] : '4242';
    $card_brand = isset($input['card_brand']) ? $input['card_brand'] : 'Visa';
    ...
    // In test mode, simulate the tap to pay transaction
    if ($test_mode) {
        // Generate simulated payment IDs
        $payment_intent_id = 'pi_tap_' . uniqid();
        $charge_id = 'ch_tap_' . uniqid();
        
        // Simulate successful payment
        $payment_status = 'succeeded';
        
        // Simulate random card brands for testing
        $card_brands = ['Visa', 'Mastercard', 'American Express', 'Discover'];
        $random_brand = $card_brands[array_rand($card_brands)];
        $simulated_last4 = str_pad(rand(0, 9999), 4, '0', STR_PAD_LEFT);
        ...
        if (true) { // Changed from if ($destination_usuario_id)
            ...
            try {
                ...
                // Get forma de pago ID for tap to pay
                ...
                $payment_description = "Tap to Pay - $random_brand ****$simulated_last4";
                ...
                if ($recipient_user_id != $usuario_id) {
                    ...
                    $conexion->doInsert(
                        ...
                        "'Payment sent via $payment_description', '$tipomov_enviado', '$recipient_user_id', '$recipient_user_id', '$amount_in_dollars', '$fechaactual', '$moneda_id', '$formapago_id', '$fechaactual', '1', '0', '$cuenta_id_sender', '$compania', '$estatus_id'"
                    );

                    // Movement for recipient (type 22 = received)
                    $conexion->doInsert(
                        "movimiento (mov_descrip, l_tipomov_id, usuario_id, usuario_iddestino, mov_monto, mov_fecha, l_moneda_id, l_formapago_id, mov_fechareg, mov_activo, mov_eliminado, cuenta_id, compania_id, l_estatus_id)",
                        "'Payment received via $payment_description', '$tipomov_recibido', '$recipient_user_id', '$recipient_user_id', '$amount_in_dollars', '$fechaactual', '$moneda_id', '$formapago_id', '$fechaactual', '1', '0', '$cuenta_id_receiver', '$compania', '$estatus_id'"
                    );
                } else {
                ...
                error_log("Tap to Pay movements created successfully");
        ...
        $response['data'] = array(
            ...
            'payment_method' => 'tap_to_pay_card',
            'card_last4' => $simulated_last4,
            'card_brand' => $random_brand,
            'test_mode' => true,
            'timestamp' => $fechaactual
        );
        
        error_log("Tap to Pay processed successfully: " . json_encode($response['data']));
        
    } else {
        // Production mode - would integrate with actual NFC reader APIs
        $response['code'] = 100;
        $response['message'] = 'Production mode not yet implemented for Tap to Pay';
    }
    
} catch (Exception $e) {
    error_log("Process Tap to Pay - Fatal Error: " . $e->getMessage());
